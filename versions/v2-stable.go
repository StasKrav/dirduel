...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "pageup":
			if m.focus == "terminal" && m.termScrollPos < len(m.termOutput)-1 {
				m.termScrollPos += 3
				if m.termScrollPos > len(m.termOutput)-1 {
					m.termScrollPos = len(m.termOutput) - 1
				}
			}
		case "pagedown":
			if m.focus == "terminal" && m.termScrollPos > 0 {
				m.termScrollPos -= 3
				if m.termScrollPos < 0 {
					m.termScrollPos = 0
				}
			}
		...
		case "enter":
			if m.focus == "terminal" {
				...
				if cmd != "" {
					...
					if result != "" {
						m.termOutput = append(m.termOutput, result)
					}
...
func (m model) View() string {
	...
	terminal := renderTerminal(m.termOutput, termH, m.width, m.focus == "terminal", m.termInput, m.termScrollPos)
...
func renderTerminal(output []string, h, w int, active bool, input string, scroll int) string {
	if h < 3 {
		h = 3
	}
	border := "-"
	...
	start := 0
	if len(output) > h-2 {
		start = len(output) - (h - 2) - scroll
		if start < 0 {
			start = 0
		}
	}
	end := len(output) - scroll
	if end > len(output) {
		end = len(output)
	}
	visible := output[start:end]

	for i, line := range visible {
		if i == len(visible)-1 && active {
