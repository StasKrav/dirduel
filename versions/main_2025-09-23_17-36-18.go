	...
	case tea.KeyMsg:
		...
		case "up":
			if m.focus == "left" && m.cursorLeft > 0 {
				...
				m.termCursorPos = len([]rune(m.termInput))
		...
		case "right":
			if m.focus == "terminal" && m.termCursorPos < len([]rune(m.termInput)) {
				m.termCursorPos++
			} else if m.focus == "left" {
				m = enterItem(m, true)
			} else if m.focus == "right" {
		...
		case "backspace":
			if m.focus == "terminal" && m.termCursorPos > 0 {
				...
				m.termInput = string(append(runes[:m.termCursorPos-1], runes[m.termCursorPos:]...))
				m.termCursorPos--
				m.termOutput[len(m.termOutput)-1] = "$ " + m.termInput
			}
		...
		default:
			if m.focus == "terminal" && len(msg.String()) == 1 {
				r := []rune(msg.String())[0]
				runes := []rune(m.termInput)
				m.termInput = string(append(runes[:m.termCursorPos], append([]rune{r}, runes[m.termCursorPos:]...)...))
				m.termCursorPos++
				m.termOutput[len(m.termOutput)-1] = "$ " + m.termInput
			}
		}
...
func renderTerminal(output []string, input string, cursor int, h, w int, active bool) string {
	if h < 3 {
		h = 3
	}
	border := "-"
