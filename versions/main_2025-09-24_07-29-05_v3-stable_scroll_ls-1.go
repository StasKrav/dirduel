...
func runCommand(cmd string) string {
	...
	// ping (через системную утилиту). На Windows флаги отличаются — возможно, нужно учитывать runtime.GOOS.
	case "ping":
		if len(parts) < 2 {
			return "Использование: ping <host>"
		}
		// cross-platform: на unix используем -c, на windows -n
		countFlag := "-c"
		if strings.Contains(strings.ToLower(os.Getenv("OS")), "windows") {
			countFlag = "-n"
		}
		c := exec.Command("ping", countFlag, "4", parts[1])
  ...
  ls [-a] [-l] [-1] [dir] - список файлов (цветной вывод)
